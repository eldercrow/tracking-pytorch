    ...
    def __init__(self, name, root, video_dir, init_rect, img_names, gt_rect,
        ...
        def convert_vot_anno_to_rect(vot_anno, type):
            ...
            if type == 'union':
                ...
                x = cx - 0.5*w
                y = cy - 0.5*h
                #print('tighter BB: ', [x, y, w, h])
                return [x, y, w, h]
            else:
                raise ValueError

        # vot_ground_truth_rect = np.empty((0,4), float)
        # for idx, gt in enumerate(self.gt_traj):
        #     vot_ground_truth_rect = np.append(vot_ground_truth_rect, np.array([convert_vot_anno_to_rect(gt, 'preserve_area')]), axis=0)

        # self.gt_traj = vot_ground_truth_rect
            ...
            for traj_file in traj_files:
                with open(traj_file, 'r') as f:
                    ...
                    for x in f.readlines():
                        try:
                            traj.append(list(map(float, x.strip().split(','))))
                        # traj = [list()
                        #         for x in f.readlines()]
                        except:
                            print(x)
